...
def hide_cursor():
    subprocess.run("setterm -cursor off | tee /dev/tty0 > /dev/null", shell=True, check=True)

def show_cursor():
    subprocess.run("setterm -cursor on | tee /dev/tty0 > /dev/null", shell=True, check=True)
